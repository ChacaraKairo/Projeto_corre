...
export const useDashboard = () => {
  ...
  const [tipoMeta, setTipoMeta] = useState<"diaria" | "semanal">("diaria");
    ...
    useCallback(() => {
      const carregarDados = async () => {
        try {
          ...
          console.log("LOG: Usuário carregado no Dashboard:", usuarioSalvo);
          ...
          const tipoMetaUser = usuarioSalvo?.tipo_meta || "diaria";
          ...
          if (veiculoSalvo) {
            ...
            if (tipoMetaUser === "diaria") {
              ...
              AND data LIKE ?
            `;
              paramsGanhos = [veiculoSalvo.id, `${dataHojeStrLocal}%`];
            } else {
              ...
              const mesSem = String(inicioSemana.getMonth() + 1).padStart(
                2,
                "0",
              );
              const diaSemStr = String(inicioSemana.getDate()).padStart(2, "0");
              ...
              AND data >= ?
            `;
              paramsGanhos = [veiculoSalvo.id, dataInicioSemanaStr];
            }

            const resultadoGanhos: any = await db.getFirstAsync(
              queryGanhos,
              paramsGanhos,
            );
          ...
          AND data LIKE ?
        `;
            const resultadoGastos: any = await db.getFirstAsync(queryGastos, [
              veiculoSalvo.id,
              `${dataHojeStrLocal}%`,
            ]);
            ...
            const resultGanhosMensal: any = await db.getFirstAsync(
              queryGanhosMensal,
              [veiculoSalvo.id, dataInicioMesStr],
            );
            ...
            const resultGastosMensal: any = await db.getFirstAsync(
              queryGastosMensal,
              [veiculoSalvo.id, dataInicioMesStr],
            );
            ...
            const ultimosRegistros: any[] = await db.getAllAsync(
  ...
  const onPressConfig = () => Alert.alert("Definições", "Abrir definições");
  const onTrocarVeiculo = () => Alert.alert("Navegação", "Ir para Garagem");
  const onIrParaOficina = () => Alert.alert("Navegação", "Ir para Manutenção");
  ...
  const salvarKm = async () => {
    if (!novoKm || isNaN(Number(novoKm)) || !veiculo) return;
    ...
    try {
      await db.runAsync("UPDATE veiculos SET km_atual = ? WHERE id = ?", [
        kmNumerico,
        veiculo.id,
      ]);
  ...
  const abrirCalculadora = () => router.push("/calculadora");
